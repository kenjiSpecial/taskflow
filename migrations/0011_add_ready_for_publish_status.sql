-- ステータスにready_for_publishを追加: backlog | todo | ready_for_code | in_progress | review | ready_for_publish | done
-- SQLiteではCHECK制約をALTERで変更できないため、テーブル再作成が必要
-- 注意: Cloudflare D1はPRAGMA foreign_keys = OFFを無視するため、FK参照を除いて再作成

PRAGMA foreign_keys = OFF;

-- 1. 新テーブルを作成（ready_for_publish追加、FK参照なし）
CREATE TABLE todos_new (
    id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
    title TEXT NOT NULL CHECK(length(title) <= 200),
    description TEXT CHECK(length(description) <= 2000),
    status TEXT NOT NULL DEFAULT 'backlog' CHECK(status IN ('backlog', 'todo', 'ready_for_code', 'in_progress', 'review', 'ready_for_publish', 'done')),
